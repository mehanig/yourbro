...
SETUP (agent machine already has API token from user):
  ...
  │                      │
  │ env: YOURBRO_TOKEN    │ ← one token, shared
  │ env: YOURBRO_SERVER_URL   │
  └──────────────────────┘
...
```
YOURBRO_TOKEN=yb_...          # same token ClawdBot uses to publish pages
YOURBRO_SERVER_URL=https://yourbro.ai   # or http://localhost for local dev
```
...
- [ ] Agent reads `YOURBRO_TOKEN` and `YOURBRO_SERVER_URL` env vars
...
5. Docker compose: add `YOURBRO_TOKEN` and `YOURBRO_SERVER_URL` to agent-server env
...
| `docker-compose.local.yml` | Add `YOURBRO_TOKEN`, `YOURBRO_SERVER_URL` to agent-server env |
| `.env.example` | Document `YOURBRO_TOKEN`, `YOURBRO_SERVER_URL` |
