...
- `sandbox="allow-scripts allow-popups allow-popups-to-escape-sandbox"` on the iframe (no `allow-same-origin`). This gives the iframe an **opaque origin**, preventing access to the parent shell's IndexedDB, cookies, and DOM. This is critical for security: page content is untrusted, and the viewer's X25519 private key lives in IndexedDB on the shell's origin. `allow-popups` + `allow-popups-to-escape-sandbox` let external links open normally. This is safe: the attacker can open real URLs (which get their own origin) but can't inject code into them, and URLs where content is attacker-controlled (`about:blank`, `data:`) inherit the opaque origin.
